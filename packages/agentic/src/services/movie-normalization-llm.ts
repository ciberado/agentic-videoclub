...
const MovieSchema = z.object({
  ...
  genre: z
    .array(z.string())
    .describe("Array of standardized genres like 'Science Fiction', 'Drama', etc."),
  rating: z
    .number()
    .min(0)
    .max(10)
    .describe('Rating out of 10 (convert from other scales if needed)'),
  ...
  themes: z
    .array(z.string())
    .describe("Array of thematic elements like 'Space exploration', 'Family drama', etc."),
});
    ...
    try {
      ...
      if (parsedResponse) {
        try {
          ...
          logLlmResponse(
            'claude-3-haiku',
            `Movie normalization fixed for "${movieDetails.title}"`,
            responseText.length,
            processingTime,
          );
...
function fixCommonParsingIssues(response: any, movieDetails: PrimeVideoMovieDetails): Movie {
  ...
  return {
    title: String(response.title || movieDetails.title || 'Unknown Title')
      .replace(/\(\d{4}\)/, '')
      .trim(),
    year: year,
    genre: Array.isArray(response.genre) ? response.genre : [],
    rating:
      typeof response.rating === 'number' && response.rating >= 0 && response.rating <= 10
        ? response.rating
        : 0,
 ...
 */
export async function normalizeMoviesBatch(
  movieDetailsList: PrimeVideoMovieDetails[],
): Promise<Movie[]> {
