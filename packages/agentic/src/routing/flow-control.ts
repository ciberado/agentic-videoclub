...
export function shouldContinueSearching(state: typeof VideoRecommendationAgentState.State): string {
  ...
  if (state.finalRecommendations.length > 0 || state.workflowCompleted) {
    logger.info('✅ Routing to END - Workflow completed', {
      ...
      reason:
        state.finalRecommendations.length > 0 ? 'recommendations_ready' : 'no_candidates_found',
    });
    return END;
  }
  ...
  if (
    ...
    movieLinksQueue.length > 0 &&
    allCandidates.length < minimumRequired
  ) {
    logger.info(
      '🔄 Routing to movie_discovery_and_data_fetching_node - Recursive discovery available',
      {
        currentDepth,
        maxDepth,
