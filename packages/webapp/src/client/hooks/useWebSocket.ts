...
import { ClientMessage, ServerMessage, LogEvent, Movie } from '../../shared/types';
...
export const useWebSocket = (
    ...
    const [connectionStatus, setConnectionStatus] = useState<WebSocketHookReturn['connectionStatus']>('disconnected');
    ...
    const handleServerMessage = (message: any): void => {
        ...
        if (message.type && ['hot', 'liveReload', 'reconnect', 'overlay', 'hash', 'ok'].includes(message.type)) {
        ...
        switch (serverMessage.type) {
            ...
            case 'node_activated':
                if (serverMessage.payload && 'nodeId' in serverMessage.payload && 'nodeName' in serverMessage.payload) {
            ...
            case 'node_error':
                if (serverMessage.payload && 'nodeId' in serverMessage.payload && 'error' in serverMessage.payload) {
            ...
            case 'progress_update':
                if (serverMessage.payload && 'nodeId' in serverMessage.payload && 'progress' in serverMessage.payload) {
                    callbacks.onProgressUpdate?.(serverMessage.payload);
                }
                break;

            case 'log_event':
                if (serverMessage.payload && 'timestamp' in serverMessage.payload && 'level' in serverMessage.payload && 'message' in serverMessage.payload) {
